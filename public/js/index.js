...
async function searchUpdate() {
  ...
  for (const userId of gumroadResults) {
  ...
  const unraw = await loadUnraw();
  for (const userId of fanboxResults) {
  ...
  for (const userId of discordResults) {
